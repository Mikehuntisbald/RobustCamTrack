...
class Matcher:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=30, n_init=3):
        ...
        self.current_track_id_map1 = {}
        self.current_track_id_map2 = {}
        #TODO 解决连续跳变问题
    def update(self, detections):
        ...
        for track_idx, detection_idx in matches:
            ...
            self.current_track_id_map1[self.tracks_1[track_idx].track_id.item()] = self.tracks_2[detection_idx].track_id.item()
            self.current_track_id_map2[self.tracks_2[detection_idx].track_id.item()] = self.tracks_1[track_idx].track_id.item()
        # print('Raw Matches')
        # print(self.current_track_id_map1)
        # print(self.current_track_id_map2)
        
        # 此逻辑应用于相机Track id跳变的情况，应用LiDAR使之稳定
        # 遍历当前的LiDAR Track
        for track_id_2, track_id_1 in self.current_track_id_map2.items():
        ...
        #     if activate_track_id_2 not in self.current_track_id_map2.keys():
        #         # hits_activate - 1
        ...
        # 如果之前不归零。在上一帧有匹配到的track1和当前帧不相符的情况下，track_id_2 in self.current_track_id_map2.keys() 为True，count不归零继而出现bug
        # 遍历 map2 的键值对出现次数字典
        for track_id_2 in self.track_id_map2_count.keys():
            # 如果当前帧的map中没有这个id
            if track_id_2 not in self.current_track_id_map2.keys():
                # count归零
                self.track_id_map2_count[track_id_2] = 0

        ### 核心
                ### 逻辑1：如果track2在当前帧的map中没有匹配，且hits2的map中有匹配，则用track2还原track1,但是这有可能导致出现两个track1 id相同的情况。因为可能当前帧track1不准，和其他的track2匹配了。
                ### 逻辑2：针对逻辑1的缺陷，如果self.current_track_id_map1里有hits2的匹配track1 id，则从self.current_track_id_map2里删除当前track2的匹配，再用track2还原track1
        ...
        for track_id_2, track_id_1 in self.current_track_id_map2.items():
            ...
            if self.track_id_map2_hits.get(track_id_2, None) is not None and self.track_id_map2_hits.get(track_id_2, None) != track_id_1 and self.track_id_map2.get(track_id_2, None) is not None:
                ...
                self.current_track_id_map2[track_id_2] = self.track_id_map2_hits[track_id_2]
                
                # 逻辑2
                # self.current_track_id_map1.get(self.track_id_map2_hits.get(track_id_2, None), None)
                # if self.current_track_id_map1.get(self.track_id_map2_hits.get(track_id_2, None), None) is not None:
                #     redundant = self.track_id_map2_hits.get(track_id_2, None)
                #     redundant_correspond = self.current_track_id_map1[redundant]
                #     self.current_track_id_map1.pop(redundant)
                #     self.tracks_1 = [track for track in self.tracks_1 if track.track_id.item() != redundant]
                #     self.current_track_id_map2.pop(redundant_correspond)
        ...
        #     if self.current_track_id_map1[track_id_1] != self.track_id_map1_hits.get(track_id_1, None) and track_id_1 in self.track_id_map1_hits.keys():
        #         track.track_id = self.current_track_id_map1[track_id_1]
        ...
        for idx, track2 in enumerate(self.tracks_2):
            ...
            if track2.track_id.item() in self.track_id_map2.keys() and track2.track_id.item() in self.track_id_map2_hits.keys():
                ...
                # if self.current_track_id_map2[track2.track_id.item()] not in self.current_track_id_map1.keys():
                #     track2.mark_missed()
                if self.current_track_id_map2.get(track2.track_id.item(), None) is None and len([track for track in self.tracks_1 if track.track_id.item() == track2.track_id.item()]) == 0 :
                    # 如果是由于lidar框多导致匹配跳变，修改hits，新建track1，删除原来的track1
                    # 如果这个没有匹配的track2在hits对应的track1 id在self.current_track_id_map1中有匹配，那么就把这个对应的匹配代替当前的track2_id
                    if self.current_track_id_map1.get(self.track_id_map2_hits[track2.track_id.item()], None) is not None:
                        track_id_2_new = self.current_track_id_map1.get(self.track_id_map2_hits[track2.track_id.item()], None)
                        ...
                        # self.current_track_id_map2[self.track_id_map2_hits[track2.track_id.item()]] = track2.track_id.item()
                        # self.current_track_id_map1[self.track_id_map2_hits[track2.track_id.item()]] = self.track_id_map2_hits[track2.track_id.item()]
                        ...
                        self.current_track_id_map1[self.track_id_map2_hits.get(track2.track_id.item(), None)] = track2.track_id.item()
                        self.current_track_id_map2[track2.track_id.item()] = self.track_id_map2_hits.get(track2.track_id.item(), None)
        # print([track.track_id for track in self.tracks_1])
# if self.current_track_id_map2.get(track2.track_id.item(), None) is None:
...
#                         self.current_track_id_map1[self.track_id_map2.get(track2.track_id.item(), None)] = track2.track_id.item()
#                         self.current_track_id_map2[track2.track_id.item()] = self.track_id_map2.get(track2.track_id.item(), None)
        print([track.track_id.item() for track in self.tracks_1])
        print(" ")            
        # print('Restored Matches')
        # print(self.current_track_id_map1)
        # print(self.current_track_id_map2)
        self.no_detection(self.current_track_id_map2)

        # # self.track_id_map1_hits = [(value, key) for key, value in self.track_id_map2_hits]
        # print(self.current_track_id_map2)
        # print("Type of self.current_track_id_map2:", type(self.current_track_id_map2))

        self.current_track_id_map1=dict(zip(self.current_track_id_map1.values(),self.current_track_id_map1.keys()))
        # self.current_track_id_map1 = [(value, key) for key, value in self.current_track_id_map2]
        # for track_idx in unmatched_tracks:
        #     if track_idx in self.current_track_id_map2.values():
        ...
        self.track_id_map1 = self.current_track_id_map1
        self.track_id_map2 = self.current_track_id_map2
    ...
    # def no_detection(self, self.current_track_id_map2):
    #     flag = False
    #     # 遍历hits_activate
    #     for activate_track_id_2 in self.track_id_map2_hits_activate.keys():
    #         # 如果hits_activate中的track id在当前帧的map中没有匹配
